...
def behr_wrap(softsrc,hardsrc,softbkg,hardbkg,softarea,hardarea,verbose=False,invertBR=False):
    ...
    behr_run = 'cd '+wd_behr+' ./BEHR'+str_ssrc+str_hsrc+str_sbkg+str_hbkg+str_sarea+str_harea+' && cd -'
    if verbose:print(behr_run)
    p = subprocess.Popen(behr_run, stdout=subprocess.PIPE, stderr=subprocess.PIPE,shell=True)
    td = StringIO(p.communicate()[0].decode('ascii'))
    df = pd.read_csv(td,sep='\t',comment='#')
    df.ix[0,1:-2] = df.ix[0,2:-1].values
    df.drop(['Median', 'Unnamed: 7', 'Lower Bound', 'Upper Bound'],axis=1,inplace=True)
    if len(df) == 4:df.drop(3,inplace=True)
...
def behrhug(df_input,verbose=False,invertBR=False):
    ...
    for index, row in df_input.iterrows():
        ...
        df_out.loc[index,['BR','BR_LB','BR_UB']] = df.loc[0,['Median','LowerB','UpperB']].values.astype(float)
        df_out.loc[index,['HR','HR_LB','HR_UB']] = df.loc[1,['Median','LowerB','UpperB']].values.astype(float)
