...
def f_xerr(cts):
    '''
    Uncertainties derived using the LR matching result
    with 3"-empirical uncertainty
    -0.319715568637 0.949380443343
    -0.275172141037 0.870004338999
    -0.270768723844 0.860176436613
    -0.300622213221 0.916053126907
    '''
    cts = cts.copy()
    cts[cts > 2000.] = 2000.
    return 10.**(-0.300622213221 * np.log10(cts) + 0.916053126907 )
...
def READXRAY(band='Full'):
    ...
    xcat = makecd(sumdf.RA.values,sumdf.DEC.values)
    return sumdf,xcat
...
def READHSC(maglim=None,xdf=None):
    ...
    hsc = pd.read_csv(home + '/surveys/xxl/cats/hsc_wide_iband_pz.csv.gz',dtype = {'# object_id':str,'specz_original_flag':str})
    #
    #hsc = pd.read_csv(home + '/Dropbox/DB_XSERVS/catalogs/hscwide_clean.csv.gz')
    ...
    cathsc = makecd(hsc.ra.values,hsc.dec.values)
    return hsc,cathsc
...
def READVIDEO(maglim=None,xdf=None):
    ...
    catvideo = makecd(video.ra.values,video.dec.values)
...
def READLSS(deep=False,xdf=None,maglim=None):
    ...
    if deep:
        ...
        lss = pd.read_csv(home + '/surveys/xxl/cats/cfhtls_wide.tsv',sep='\t',comment='#')
    ...
    catlss = makecd(lss.ra.value,lss.dec.values)
...
if READIZY:
    ...
    cathscriz = makecd(hscriz.ra.values,hscriz.dec.values)
...
def READLSS(deep=False,xdf=None,maglim=None):
    ...
    if maglim is not None:
        lss = lss[ (lss.imag <= maglim[0]) & (lss.imag >= maglim[1])]
    lss.set_index('CFHTLS',inplace=True)
    catlss = makecd(lss.ra.values,lss.dec.values)
    lss['numid'] = pd.Series(range(len(lss)),index=lss.index)
    return lss, catlss
def READSERVS():
    fname = '/cuc36/xxl/multiwavelength/vacc/xmm/121212/xmm-irac12-sextractor.fits.gz'
    ...
    servs['numid'] = pd.Series(range(len(servs)), index=servs.index)
    servs.rename(columns={'RA_12':'ra','DEC_12':'dec'},inplace=True)
    catservs = makecd(servs.ra.values,servs.dec.values)
    return servs,catservs#Check Data Fusion
'''